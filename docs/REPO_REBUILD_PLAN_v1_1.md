...
- Suggested repo name: `eastie-curriculum-app`
- Suggested local path: `/Users/Lucia/Desktop/eastie_curriculum_app/`
...
- generated TS runtime snapshots
  - include generated curriculum TypeScript snapshots in the short term if the app needs them to build and render the current routes;
  - treat them as runtime snapshots, not as hand-edited source;
  - keep sync scripts as the mechanism that regenerates them.
...
- `curriculum_release/`:
  ...
  - this workspace is a release source sample and traceability area, not a web runtime dependency;
  - only include it later if PM explicitly decides the clean repo should become the formal release content repo.
- Generated TS:
  - should be treated as build-time or sync-time snapshot output;
  - include them in the new repo for the short term because the current app runtime depends on generated curriculum data;
  - do not treat them as editable source;
  - regenerate them through sync scripts instead of manual editing.
...
- generated TS snapshots required for initial route rendering.
...
- Whether `docs/` is fully included in the repo baseline.
...
- `curriculum_release/` in repo: no for the initial clean repo baseline.
- Generated curriculum snapshots in repo: yes for the initial baseline if required for runtime/bootstrap and route rendering.
- Docs in repo: yes, include the full `docs/` set.
